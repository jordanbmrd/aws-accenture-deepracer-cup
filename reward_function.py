...
def reward_function(params):
    ...
    Used in Brest (FR) AWS x Accenture Cup
    Team: Tokyo Drift
    December 14th 2023
    Based on the reward function used in USYD 2020 Qualifier in the 2019 Championship track by the team IndestruciRacer (https://github.com/MatthewSuntup/DeepRacer)
    '''
    ...
    DIST_THRESHOLD = 1.0    # metres
    SPEED_THRESHOLD = 1.8   # m/s
    STRAIGHT_SPEED_BONUS = 0.2
    SPECIFIC_ACTION_REWARD = 0.5
    ...
    # More reward for fast speed in straight lines
    if go_fast and abs(params['steering_angle']) < 5:
        reward += STRAIGHT_SPEED_BONUS
    if params['steering_angle'] == 0 and params['speed'] == 4:
        reward += SPECIFIC_ACTION_REWARD
